...
public class PayoutTest16 extends Payout implements PayoutCase {
    ...
    private final int testCase = 16, payoutOdds = 1, betOption = 6;
    ...
    public void setBetOption() {
        ...
        if (DataTypeHandler.findInArray(14, testCaseList)) return;
        if (DataTypeHandler.findInArray(15, testCaseList)) return;
    ...
    public void computeTestCase(int[] roundResult) {
        if (!DataTypeHandler.findInArray(testCase, testCaseList) &&
                !DataTypeHandler.findInArray(14, testCaseList) &&
                !DataTypeHandler.findInArray(15, testCaseList)) return;
        ...
        for (int i = 0; i < betOption; i++) {
            if (RoundCondition.isSpecificTripleWin(roundResult, (i + 1))) {
                ...
                bet += winBet;
                payout += winBet + (winBet * (payoutOdds * 3));
                addWin(winBet, (payoutOdds * 3));
            }
        }
    }
